 ...
 * @internal
 */
export const lifeCycleToSubscribedEvent = new Map([['load', 'browsingContext.load'], ['domcontentloaded', 'browsingContext.domContentLoaded']]);
/**
 * @internal
 */
const lifeCycleToReadinessState = new Map([['load', "complete" /* Bidi.BrowsingContext.ReadinessState.Complete */], ['domcontentloaded', "interactive" /* Bidi.BrowsingContext.ReadinessState.Interactive */]]);
/**
 * @internal
 */
...
export class CDPSessionWrapper extends CDPSession {
  ...
  async send(method, ...paramArgs) {
    ...
    const {
      result
    } = await this.#context.connection.send('cdp.sendCommand', {
      method: method,
      params: paramArgs[0],
      session
    });
...
export class BrowsingContext extends Realm {
  ...
  async goto(url, options) {
    ...
    try {
      const {
        result
      } = await waitWithTimeout(this.connection.send('browsingContext.navigate', {
        url: url,
        context: this.#id,
        wait: readinessState
      }), 'Navigation', timeout);
  ...
  async setContent(html, options) {
    ...
    await Promise.all([setPageContent(this, html), waitWithTimeout(new Promise(resolve => {
