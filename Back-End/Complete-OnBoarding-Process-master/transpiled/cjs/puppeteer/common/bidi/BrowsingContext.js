 ...
 * @internal
 */
exports.lifeCycleToSubscribedEvent = new Map([['load', 'browsingContext.load'], ['domcontentloaded', 'browsingContext.domContentLoaded']]);
/**
 * @internal
 */
...
class BrowsingContext extends Realm_js_1.Realm {
  ...
  async goto(url, options) {
    ...
    try {
      const {
        result
      } = await (0, util_js_1.waitWithTimeout)(this.connection.send('browsingContext.navigate', {
        url: url,
        context: this.#id,
        wait: readinessState
      }), 'Navigation', timeout);
