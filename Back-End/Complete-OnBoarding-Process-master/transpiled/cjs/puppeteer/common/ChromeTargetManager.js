...
class ChromeTargetManager extends EventEmitter_js_1.EventEmitter {
  ...
  constructor(connection, targetFactory, targetFilterCallback, waitForInitiallyDiscoveredTargets = true) {
    ...
    }).then(this.#storeExistingTargetsForInit).catch(util_js_1.debugError);
  }
  ...
  #onAttachedToTarget = async (parentSession, event) => {
    ...
    for (const interceptor of this.#targetInterceptors.get(parentSession) || []) {
      ...
      interceptor(target, parentSession instanceof Connection_js_1.Connection ? null : this.#attachedTargetsBySessionId.get(parentSession.id()));
    }
